...
const Processor = require('jazzjs');
...
module.exports = (config = false) => {
  ...
  const missing = ({ source, file }) => {
    ...
    if (strict) {
      throw new Error(
        `${prefix} Unable to find .${classes.join(', .')} in "${file}"`,
      );
    }

    classes.forEach((key) => warn(`Unable to find .${key} in ${file}`));

    // Turn all missing values into strings so nothing explodes
    return source.replace(
      new RegExp(
        `(\\b)(${missed.map((ref) => escape(ref)).join('|')})(\\b)`,
        'g',
      ),
  ...
  const markup = async ({ content, filename }) => {
    ...
    if (links && style) {
      throw new Error(
        `${prefix} Use <style> OR <link>, but not both in "${file}"`,
      );
    }
    ...
    if (style) {
      ...
      try {
        result = await processor.string(filename, style[1]);
      } catch (e) {
    ...
    if (classKeys.length) {
      ...
      const matches = source.match(
        new RegExp(`class={css\\.(?:${selectors})}`, 'g'),
      );
