...
import { IdentifierScope } from './utils/infer-scope';
...
// const isFromRule = (n: Ast.StatementNode): n is Ast.FromAtRule =>
//   n.type === 'atrule' && n.name === 'from';
...
export default class Evaluator
  ...
  visitExportRule(node: Ast.ExportAtRule, exports: ModuleMembers) {
    ...
    if (!otherExports) {
      throw new Error(`Could not resolve module ${node.request}`);
    }
      ...
      if (argument.type === 'numeric') argument.value *= -1;
        ...
        argument = new MathFunctionValue(
          ...
          new BinaryMathExpression(
            new NumericValue(-1),
            '*',
            argument.type === 'math-function' && argument.callee === 'calc'
              ? argument.args[0]
              : argument,
          ),
        );
  ...
  visitCallExpression(node: Ast.CallExpression): Value {
    ...
    if (member) {
      ...
      throw node.error(`Undefined function ${node.callee}`);
    }
    // }

    if (args.keywords.size) {
      throw node.error(`Undefined function call with keywords`);
    }
  ...
  private parseKeyframesParams(node: Ast.CssAtRule): void {
    ...
    const selector = this.parser.selector(node.params);
